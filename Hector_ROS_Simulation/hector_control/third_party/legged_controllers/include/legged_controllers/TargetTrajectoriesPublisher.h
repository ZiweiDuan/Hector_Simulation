...
class TargetTrajectoriesPublisher final {
 public:
  ...
  TargetTrajectoriesPublisher(::ros::NodeHandle& nh, const std::string& topicPrefix, CmdToTargetTrajectories goalToTargetTrajectories,
    ...
    // goal subscriber:  each time receive a target ("goal") pose, compute target trajectory and then publsh it. 
    ...
    // command velocity subscriber: each time receive a cmd_vel, compute target trajectory and then publsh it. 
 ...
 private:
  CmdToTargetTrajectories goalToTargetTrajectories_, cmdVelToTargetTrajectories_;   // callables 
